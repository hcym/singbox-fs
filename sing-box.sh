...
fetch_quicktunnel_domain() {
  ...
  until [ -n "$ARGO_DOMAIN" ]; do
    if [ -z "$CLOUDFLARED_PID" ]; then
      if [ "$SYSTEM" = 'Alpine' ]; then
        CLOUDFLARED_PID=$(ps -ef | awk "/${WORK_DIR////\\\/}\/cloudflared/{print \$1; exit}")
      elif [ "$IS_CENTOS" = 'CentOS7' ]; then
        CLOUDFLARED_PID=$(ps -ef | awk "/$(eval echo "${WORK_DIR////\\\/}")\/cloudflared/{print \$2; exit}")
      else
        CLOUDFLARED_PID=$(ps -ef | awk "/${WORK_DIR////\\\/}\/cloudflared/{print \$2; exit}")
      fi
    fi
    [ -z "$METRICS_ADDRESS" ] && METRICS_ADDRESS=$(ss -nltp | grep "pid=$CLOUDFLARED_PID" | awk '{print $4}')
...
export_list() {
  ...
  [[ 'is_json_argo is_token_argo' =~ "$ARGO_TYPE" ]] && SUBSCRIBE_ADDRESS="https://$ARGO_DOMAIN" || SUBSCRIBE_ADDRESS="http://${SERVER_IP_1}:${PORT_NGINX}"
  ...
  if [[ "${STATUS[0]}" =~ $(text 27)|$(text 28) ]]; then
    ...
    if [[ "${STATUS[1]}" =~ $(text 27)|$(text 28) ]]; then
      ...
      if [ "${STATUS[1]}" = "$(text 28)" ]; then
        if [ "$SYSTEM" = 'Alpine' ]; then
          ARGO_PID=$(pidof cloudflared | sed -n 1p)
        else
          ARGO_SYSTEMCTL_STATUS=$(systemctl status argo)
          ARGO_PID=$(awk '/PID/{print $3}' <<< "$ARGO_SYSTEMCTL_STATUS")
          ARGO_RUNTIME="$(text 57): $(awk '/Active:/{for (i=5;i<=NF;i++)printf("%s ", $i);print ""}' <<< "$ARGO_SYSTEMCTL_STATUS")"
        fi
        ARGO_MEMORY_USAGE="$(text 58): $(awk '/VmRSS/{printf "%.1f\n", $2/1024}' /proc/$ARGO_PID/status) MB"
      fi
    fi
    ...
    if [ "${STATUS[0]}" = "$(text 28)" ]; then
      if [ "$SYSTEM" = 'Alpine' ]; then
        SING_BOX_PID=$(pidof sing-box | sed -n 1p)
      else
        SING_BOX_SYSTEMCTL_STATUS=$(systemctl status sing-box)
        SING_BOX_PID=$(awk '/PID/{print $3}' <<< "$SING_BOX_SYSTEMCTL_STATUS")
        SING_BOX_RUNTIME="$(text 57): $(awk '/Active:/{for (i=5;i<=NF;i++)printf("%s ", $i);print ""}' <<< "$SING_BOX_SYSTEMCTL_STATUS")"
      fi
      SING_BOX_MEMORY_USAGE="$(text 58): $(awk '/VmRSS/{printf "%.1f\n", $2/1024}' /proc/$SING_BOX_PID/status) MB"
    fi

    # 查 Nginx 版本号
    [ $(type -p nginx) ] && NGINX_VERSION=$(nginx -v 2>&1 | sed "s#.*/#Version: #")
    [ "$SYSTEM" = 'Alpine' ] && NGINX_PID=$(ps -ef | awk "/${WORK_DIR////\\\/}\/nginx\.conf/{print \$1}") || NGINX_PID=$(ps -ef | awk "/${WORK_DIR////\\\/}\/nginx\.conf/{print \$2}")
